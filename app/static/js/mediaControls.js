...
function captureCode() {
    ...
    $.ajax({
        ...
        contentType: "application/json",
              success: function(response) {
            if (isResponseEmptyOrWhitespace(response)) { // TODO: Display error message on capture fail
                displayCapture(response, captureTimestamp);
                sendCaptureUpdate(captureTimestamp, response);
                captureFailTone.currentTime = 0;
                captureFailTone.play();
            } else {
                displayCapture(response, captureTimestamp);
                sendCaptureUpdate(captureTimestamp, response);
                captureSuccessTone.currentTime = 0;
                captureSuccessTone.play();
            }
        },
...
/**
 * Removes empty spaces from captures with no content
 */

function isResponseEmptyOrWhitespace(response) {
